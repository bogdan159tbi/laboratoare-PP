...
-- implementare proprie pt functia elem
member :: (Foldable t, Eq a) => a -> t a -> Bool
member x = foldl (opp x) False where
                opp x False domain = x == domain
                opp _ True _ = True 
addUniqueElem :: Eq a => a -> [a] -> [a]
addUniqueElem x acc 
                | x `member` acc = acc
                | otherwise = x:acc
getListOfDomains :: [String] -> [String]
getListOfDomains = map(head.tail.splitEmail)

domainsProfu :: [String] -> [String]
domainsProfu =  (foldr addUniqueElem []).getListOfDomains
...
splitBy separator = foldr op [] where
                        op c [] 
                                | (c == separator) = []
                                | otherwise = [[c]]
                        op c (str:acc) 
